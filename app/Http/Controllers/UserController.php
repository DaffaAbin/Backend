...
class UserController extends Controller
{
    public function vote(Request $request, $IdPemilih)
    {
        ...
        if ($status && $status->IdStatus == 1) {
            ...
            //mengecek apakah id_calon yang di berikan ada pada tabel kandidat atau tidak
    ...
    public function dashboard($IdPemilih)
    {
        ...
        $votemasuk = DB::table('voting')
            ->join('users', 'voting.id_user', '=', 'users.id')
            ->join('calons', 'voting.id_calon', '=', 'calons.id_calon')
            ->join('importpemilih', 'voting.IdPemilih', '=', 'importpemilih.IdPemilih')
            ->select(
                'voting.IdVoting',
                'users.nama as NamaPemilih',
                DB::raw('(SELECT nama FROM user WHERE users.id = calons.id_user) AS VoteKandidat'),
        ...
        $totalvotekandidat = DB::table('voting')
            ->join('calons', 'voting.id_calon', '=', 'calons.id_calon')
            ->join('users', 'calons.id_user', '=', 'users.id')
            ->select(
                'voting.id_calon',
                DB::raw('(SELECT nama FROM user WHERE users.id = calons.id_user) AS VoteKandidat'),
            ...
            ->groupBy('voting.id_calon', 'calons.id_user')
            ->get();
        ...
        $belumvote = DB::table('user')
            ...
            ->where('role', '<>', 'admin')
            ->whereNotIn('id_user', function ($query) use ($IdPemilih) {
